...
allPassed =
  checkFileContent('src/wasm/liboqs_kem.ts', 'instanceCache', 'Instance caching (KEM)') && allPassed
allPassed =
  checkFileContent('src/wasm/liboqs_dsa.ts', 'instanceCache', 'Instance caching (DSA)') && allPassed

// 5. Check Log Rotation
allPassed =
  checkFileContent(
    'src/components/Playground/PlaygroundContext.tsx',
    'updated.slice(0, 1000)',
    'Log rotation'
  ) && allPassed
