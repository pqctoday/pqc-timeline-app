...
import { Shield, Lock, FileSignature, ArrowRight, ArrowUpDown, ArrowUp, ArrowDown } from 'lucide-react';
            ...
            <div className="glass-panel overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-center border-collapse" style={{ tableLayout: 'fixed' }}>
                        <caption className="sr-only">Algorithm transition from classical to post-quantum cryptography, showing function type, classical algorithm, PQC alternative, and transition timeline</caption>
                        <thead>
                            <tr className="bg-white/10 text-muted text-sm uppercase tracking-wider border-b border-white/10">
                                <th
                                    scope="col"
                                    aria-sort={sortColumn === 'function' ? (sortDirection === 'asc' ? 'ascending' : 'descending') : 'none'}
                                ...
                                <th
                                    scope="col"
                                    aria-sort={sortColumn === 'classical' ? (sortDirection === 'asc' ? 'ascending' : 'descending') : 'none'}
                                    className="font-bold whitespace-nowrap relative hover:bg-white/5 transition-colors select-none p-0"
                                ...
                                <th
                                    scope="col"
                                    aria-sort={sortColumn === 'pqc' ? (sortDirection === 'asc' ? 'ascending' : 'descending') : 'none'}
                                    className="font-bold whitespace-nowrap relative hover:bg-white/5 transition-colors select-none p-0"
                                ...
                                <th
                                    scope="col"
                                    aria-sort={sortColumn === 'deprecation' ? (sortDirection === 'asc' ? 'ascending' : 'descending') : 'none'}
                                    className="font-bold whitespace-nowrap relative hover:bg-white/5 transition-colors select-none p-0"
                                ...
                                >
                                    <td className="px-4 py-8" style={{ width: `${columnWidths.function}px` }}>
                                        <div className="flex items-center justify-center gap-4 text-primary font-medium text-xl overflow-hidden">
                                            {algo.function.includes('Signature') ? <FileSignature size={24} className="flex-shrink-0" /> : <Lock size={24} className="flex-shrink-0" />}
                                            <span className="truncate">{algo.function}</span>
                                        </div>
                                    </td>
                                    <td className="px-4 py-8" style={{ width: `${columnWidths.classical}px` }}>
                                        <div className="flex flex-col items-center gap-2 overflow-hidden">
                                            <span className="text-white font-bold text-xl truncate w-full text-center">{algo.classical}</span>
                                            {algo.keySize && (
