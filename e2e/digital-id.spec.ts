...
test.describe('EUDI Digital Identity Wallet Module', () => {
    ...
    test('Completes PID Issuance and Relying Party Flows', async ({ page, browserName }) => {
        ...
        await test.step('Attestation Issuer Flow', async () => {
            ...
            await expect(page.getByRole('button', { name: 'Issue Diploma' })).toBeVisible({ timeout: 60000 })
        ...
        await test.step('QES Provider Flow', async () => {
            ...
            await expect(page.getByText('The document has been signed with a Qualified Electronic Signature')).toBeVisible()
        ...
        await test.step('Relying Party Flow', async () => {
            ...
            await expect(page.getByText('Generating Zero-Knowledge Proof...')).toBeVisible({ timeout: 5000 })
            await expect(page.getByRole('button', { name: 'Check Verification Result' })).toBeVisible({ timeout: 30000 })
