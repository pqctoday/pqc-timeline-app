...
export const useSymmetricOperations = ({
  ...
  const runSymmetricOperation = async (
    type: 'encrypt' | 'decrypt' | 'symEncrypt' | 'symDecrypt'
  ) => {
    setLoading(true)
    setError(null)
    ...
    try {
      ...
      if (type === 'symEncrypt' || type === 'symDecrypt')
        selectedKey = keyStore.find((k) => k.id === selectedSymKeyId)
