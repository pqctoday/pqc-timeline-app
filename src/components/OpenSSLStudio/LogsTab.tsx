                        ...
                        {structuredLogs.length === 0 ? (
                            ...
                            structuredLogs.map((log) => (
                                <tr key={log.id} className="hover:bg-white/5 transition-colors group">
                                    ...
                                    <td className="p-3 text-xs font-medium text-white align-top">{log.operationType}</td>
                                    <td className="p-3 text-xs text-muted break-all align-top font-mono">
                                        <div className="text-white/80">{log.command}</div>
                                        {log.details && <div className="text-[10px] text-white/40 mt-1">{log.details}</div>}
                                    </td>
                                        ...
                                        {log.fileName ? (
                                            <div>
                                                ...
                                                {log.fileSize !== undefined && (
                                                    <div className="text-[10px] text-white/40">{formatSize(log.fileSize)}</div>
                                                )}
                                            </div>
                                        ) : (
