...
const INITIAL_ATTRIBUTES: X509Attribute[] = [
    { id: 'CN', label: 'Common Name', oid: 'CN', status: 'mandatory', value: '', enabled: true, placeholder: 'e.g., example.com', description: 'The fully qualified domain name (FQDN) of your server.', elementType: 'SubjectRDN' },
];
...
export const CertSigner: React.FC<CertSignerProps> = ({ onComplete }) => {
    ...
    const rootCAs = artifacts.certificates.filter(c => c.tags.includes('root') && c.tags.includes('ca'));
    ...
    useEffect(() => {
        ...
        }).filter(name => name.startsWith('Cert') && name.endsWith('.csv'));
    ...
    const handleProfileSelect = async (filename: string) => {
        ...
        try {
            ...
            // If a CSR is already selected, try to re-import values
            if (selectedCsrId) {
    ...
    const handleSign = async () => {
        ...
        try {
            ...
            const caCert = rootCAs.find(c => c.keyPairId === selectedKeyId) ||
            ...
            if (caCert) {
                ...
                setOutput(prev => prev + `Tip: Go back to Step 2 and generate a Root CA using "${caKey?.name}".\n`);
            }
...
`;
            ...
            const hasKeyUsage = attributes.some(a => a.enabled && (a.id === 'keyUsage' || a.label === 'keyUsage'));
            if (!hasKeyUsage) {
                                ...
                                <tr key={attr.id} className={`border-b border-white/5 hover:bg-white/5 transition-colors ${!attr.enabled ? 'opacity-50' : ''}`}>
                                    ...
                                    <td className="p-3">
                                        <input
                                            ...
                                            disabled={!attr.enabled || attr.source === 'CSR'}
