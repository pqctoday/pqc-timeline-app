...
const toPKCS8 = (privKey: Uint8Array): string => {
    ...
    const pkcs8 = [0x30, ...encodeLen(version.length + algoId.length + privOctet.length), ...version, ...algoId, ...privOctet]
...
const LMS_TYPES = [
    ...
    { name: 'LMS_SHA256_M32_H10', value: 0x06, description: 'Height 10 (Merkle Tree) - Default', slug: 'h10' },
...
const LMOTS_TYPES = [
    ...
    { name: 'LMOTS_SHA256_N32_W8', value: 0x04, description: 'Winternitz Width 8 - Default', slug: 'w8' },
]
...
export const LmsConfig: React.FC<LmsConfigProps> = ({
    ...
    useEffect(() => {
        ...
        if (mode === 'verify') {
            ...
            if (currentKey && currentKey.endsWith('.key')) {
                ...
                if (lmsPub) {
                    ...
                    const lmsPem = pemFiles.find((f: { name: string }) => f.name.toLowerCase().includes('lms'))
            ...
            if (!currentKey && privFiles.length > 0) {
                const lmsPriv = privFiles.find((f: { name: string }) => f.name.toLowerCase().includes('lms'))
                if (lmsPriv) {
        ...
        const sigFiles = files.filter((f: { name: string }) => f.name.endsWith('.sig') || f.name.endsWith('.bin'))
        if (!lmsSigFile && sigFiles.length > 0) {
            const lmsSig = sigFiles.find((f: { name: string }) => f.name.toLowerCase().includes('lms') && f.name.toLowerCase().includes('sig'))
        ...
        <div className="space-y-4 animate-fade-in">
            ...
            <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-3">
                ...
                <p className="text-xs text-muted-foreground pl-6 mb-3">
                    LMS uses WebAssembly for key generation and signing. OpenSSL 3.6.0 CLI supports verification only.
                </p>
                <button
            ...
            {mode === 'generate' && (
                <div className="space-y-3 pb-4 border-b border-white/10">
                    <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-green-400 uppercase tracking-wider">Generator</span>
                    </div>

                    {/* Custom Parameters */}
                    <div className="grid grid-cols-2 gap-2">
                        <div>
                            <label className="text-xs text-muted-foreground block mb-1">LMS Algorithm (Height)</label>
                            <select
                                value={lmsType}
                                onChange={(e) => setLmsType(Number(e.target.value))}
                                className="w-full bg-background border border-input rounded text-xs px-2 py-1"
                            >
                                {LMS_TYPES.map(t => <option key={t.value} value={t.value}>{t.name}</option>)}
                            </select>
                        </div>
                        <div>
                            <label className="text-xs text-muted-foreground block mb-1">LM-OTS Algorithm (Width)</label>
                            <select
                                value={lmotsType}
                                onChange={(e) => setLmotsType(Number(e.target.value))}
                                className="w-full bg-background border border-input rounded text-xs px-2 py-1"
                            >
                                {LMOTS_TYPES.map(t => <option key={t.value} value={t.value}>{t.name}</option>)}
                            </select>
                        </div>
                    </div>

                    <div className="text-[10px] text-muted-foreground">
                        Naming convention: <code className="bg-muted px-1 rounded">lms_&lt;height&gt;_&lt;ots&gt;_&lt;timestamp&gt;.key</code>
                    </div>
                    ...
                    <button
                        onClick={async () => {
                            try {
                                ...
                                const { publicKey, privateKey } = await lmsService.generateKeypair(lmsType, lmotsType)
                                ...
                                console.error(e)
                                addLog('error', 'Key generation failed: ' + (e instanceof Error ? e.message : String(e)))
                            }
                        }}
                    ...
                    <button
                        ...
                        onClick={async () => {
                            try {
                                ...
                                const { signature, updatedPrivateKey } = await lmsService.sign(privRaw, toUint8(dataFile.content))
                                ...
                                addLog('info', `WARNING: LMS Private Key State Updated (${privKeyFile.name}). Do not revert to old key state!`)

                            } catch (e: unknown) {
                                console.error(e)
                                addLog('error', 'LMS Signing failed: ' + (e instanceof Error ? e.message : String(e)))
                            }
                        }}
            ...
            {mode === 'verify' && (
                <div className="space-y-3 pb-4 border-b border-white/10">
                    <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-purple-400 uppercase tracking-wider">Verifier</span>
                    </div>
                    <button
                        ...
                        onClick={async () => {
                            try {
                                ...
                                const valid = await lmsService.verify(
                                    pubRaw,
                                    msgBytes,
                                    sigBytes
                                )
                                ...
                                console.error(e)
                                addLog('error', 'Verification failed: ' + (e instanceof Error ? e.message : String(e)))
                            }
                        }}
            ...
            {mode === 'verify' && (
                <div className="space-y-3">
                    ...
                    >
                        ...
                        {files
                            .filter((f: { name: string; size: number }) => f.name.endsWith('.pem') || f.name.endsWith('.pub'))
                            .map((f: { name: string; size: number }) => (
                                <option key={f.name} value={f.name}>
                                    {f.name} ({f.size} bytes)
                                </option>
            ...
            {mode === 'verify' && (
                <div className="space-y-3">
                    ...
                    >
                        ...
                        {files
                            .filter((f: { name: string; size: number }) => f.name.endsWith('.sig') || f.name.endsWith('.bin'))
                            .map((f: { name: string; size: number }) => (
                                <option key={f.name} value={f.name}>
                                    {f.name} ({f.size} bytes)
                                </option>
