...
files.forEach((file) => {
  ...
  content = content.replace(
    /hsm_generateAESKey\s*\(\s*([^,]+),\s*([^,]+),\s*([^,)]+)\s*\)/g,
    (match, p1, p2, p3) => {
