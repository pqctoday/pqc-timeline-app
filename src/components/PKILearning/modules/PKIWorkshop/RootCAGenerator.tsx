...
const certProfiles = import.meta.glob('../../../../data/x509_profiles/Cert*.csv', { query: '?raw', import: 'default' });
...
const ALGORITHMS: AlgorithmOption[] = [
    { id: 'rsa', name: 'RSA', group: 'Classic', genCommand: 'openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:4096', keySizeLabel: '4096 bits' },
    { id: 'ecdsa', name: 'ECDSA (P-521)', group: 'Classic', genCommand: 'openssl genpkey -algorithm EC -pkeyopt ec_paramgen_curve:P-521', keySizeLabel: '521 bits' },
    { id: 'eddsa', name: 'EdDSA (Ed448)', group: 'Classic', genCommand: 'openssl genpkey -algorithm ED448', keySizeLabel: '456 bits' },
    { id: 'mldsa', name: 'ML-DSA-87 (Dilithium)', group: 'Quantum-Safe', genCommand: 'openssl genpkey -algorithm ml-dsa-87', keySizeLabel: 'Level 5' },
    { id: 'slhdsa', name: 'SLH-DSA-SHA2-256s (SPHINCS+)', group: 'Quantum-Safe', genCommand: 'openssl genpkey -algorithm slh-dsa-sha2-256s', keySizeLabel: 'Level 5' },
];

const INITIAL_ATTRIBUTES: X509Attribute[] = [
    { id: 'CN', label: 'Common Name', oid: 'CN', status: 'mandatory', value: '', enabled: true, placeholder: 'e.g., Root CA', description: 'The name of the Root CA.', elementType: 'SubjectRDN' },
];
...
export const RootCAGenerator: React.FC<RootCAGeneratorProps> = ({ onComplete }) => {
    ...
    useEffect(() => {
        ...
        const profiles = Object.keys(certProfiles).map(path => {
...
`;
            ...
            const hasBasicConstraints = attributes.some(a => a.enabled && (a.id === 'basicConstraints' || a.label === 'basicConstraints'));
                                        ...
                                        <td className="pt-0 pb-3 align-middle">
                                            ...
                                            >
                                                ...
                                                {availableProfiles.map(profile => (
                                                    <option key={profile} value={profile}>{profile.replace('Cert-', '').replace('.csv', '')}</option>
                                                ))}
                                            </select>
                                        </td>
                                    </tr>
