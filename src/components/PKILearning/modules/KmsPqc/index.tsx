...
import { Trash2, KeyRound, Lock, Shuffle, CalendarClock, Globe } from 'lucide-react'
import { KmsPqcIntroduction } from './components/KmsPqcIntroduction'
import { KmsPqcExercises, type WorkshopConfig } from './components/KmsPqcExercises'
...
export const KmsPqcModule: React.FC = () => {
  ...
  const handleReset = () => {
    if (confirm('Restart KMS & PQC Key Management Module?')) {
      setCurrentPart(0)
      setConfigKey((prev) => prev + 1)
      startTimeRef.current = Date.now()
      updateModuleProgress(MODULE_ID, {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gradient">KMS &amp; PQC Key Management</h1>
          <p className="text-muted-foreground mt-2">
            Master PQC key management patterns: envelope encryption with ML-KEM, hybrid key
            wrapping, and cross-provider KMS strategies.
          </p>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        ...
        <TabsContent value="references">
          <ModuleReferencesTab moduleId="kms-pqc" />
        </TabsContent>
