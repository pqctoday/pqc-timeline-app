...
export const scrapeCC = async (): Promise<ComplianceRecord[]> => {
    try {
        ...
        console.log(`[CC] Processing ${candidateRecords.length} recent records (>= ${cutoffDate.toISOString().split('T')[0]}). This may take a while...`);
        ...
        for (let i = 0; i < candidateRecords.length; i += BATCH_SIZE) {
            ...
            const batchPromises = batch.map(async (row: any) => {
                ...
                const mainLink = row['Certification Report URL'] || pdfUrl || 'https://www.commoncriteriaportal.org/products/';
                ...
                if (pdfUrl) {
                    try {
                        ...
                        const labMatch = pdfText.text.match(/(?:Evaluation Facility|ITSEF|Evaluation Body)\s*:?\s*([^\n\r,]+)/i);
                        if (labMatch) {
