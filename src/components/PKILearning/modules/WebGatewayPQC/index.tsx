...
import { Trash2, Network, Split, Calculator, RefreshCw, ClipboardCheck } from 'lucide-react'
...
const MODULE_ID = 'web-gateway-pqc'
...
export const WebGatewayPQCModule: React.FC = () => {
  ...
  const handleReset = () => {
    if (confirm('Restart Web Gateway PQC Module?')) {
      setCurrentPart(0)
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gradient">Web Gateway PQC</h1>
          <p className="text-muted-foreground mt-2">
            PQC deployment at the infrastructure edge &mdash; TLS termination patterns, certificate
            lifecycle at scale, and vendor migration paths.
          </p>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        ...
        <TabsContent value="workshop">
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="glass-panel p-4 sm:p-6 md:p-8 min-h-[400px] md:min-h-[600px] animate-fade-in">
              ...
              {currentPart === 2 && <HandshakeBudgetCalculator key={`budget-${configKey}`} />}
              {currentPart === 3 && <CertRotationPlanner key={`cert-rotation-${configKey}`} />}
              {currentPart === 4 && <VendorReadinessMatrix key={`vendor-${configKey}`} />}
            </div>
        ...
        <TabsContent value="exercises">
          <WebGatewayExercises onNavigateToWorkshop={navigateToWorkshop} />
        </TabsContent>
