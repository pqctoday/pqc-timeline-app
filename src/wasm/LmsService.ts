...
interface LmsModule {
    _generate_keypair_wasm: (pubPtr: number, privPtr: number, lmsType: number, lmotsType: number) => number;
    _sign_message_wasm: (privPtr: number, pubPtr: number, msgPtr: number, msgLen: number, sigPtr: number) => number;
    _verify_wasm: (pubPtr: number, msgPtr: number, msgLen: number, sigPtr: number, sigLen: number) => number;
...
export class LmsService {
    ...
    async generateKeypair(lmsType: number, lmotsType: number): Promise<{ publicKey: Uint8Array, privateKey: Uint8Array }> {
        await this.init();
        const m = this.module!;
    ...
    async sign(privateKey: Uint8Array, message: Uint8Array): Promise<{ signature: Uint8Array, updatedPrivateKey: Uint8Array }> {
        await this.init();
        const m = this.module!;
        ...
        try {
            ...
            const updatedPrivateKey = new Uint8Array(m.HEAPU8.subarray(privPtr, privPtr + PRIV_LEN)).slice();
