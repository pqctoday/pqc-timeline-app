...
test.describe('Compliance Source Validation', () => {
    ...
    test('NIST FIPS 140-3 - Search List (Browser)', async ({ page }) => {
        ...
        await page.goto('https://csrc.nist.gov/projects/cryptographic-module-validation-program/validated-modules/search/all?searchMode=Advanced&Standard=FIPS+140-3&ValidationStatus=Active&SecurityLevel=3', { waitUntil: 'domcontentloaded' });
    ...
    test('NIST FIPS 140-3 - Detail (Browser)', async ({ page }) => {
        // Stable Cert ID
        await page.goto('https://csrc.nist.gov/projects/cryptographic-module-validation-program/certificate/4282', { waitUntil: 'domcontentloaded' });
    ...
    test('NIST ACVP - Search List (Browser)', async ({ page }) => {
        await page.goto('https://csrc.nist.gov/projects/cryptographic-algorithm-validation-program/validation-search?searchMode=implementation&productType=-1&algorithm=179&algorithm=180&ipp=25', { waitUntil: 'domcontentloaded' });
    ...
    test('Common Criteria - CSV Download', async ({ request }) => {
        const response = await request.get('https://www.commoncriteriaportal.org/products/certified_products.csv');
    ...
    test('Common Criteria - PDF Report', async ({ request }) => {
        const response = await request.get('https://www.commoncriteriaportal.org/files/epfiles/0675a_pdf.pdf');
        expect(response.status()).toBe(200);
    ...
    test('NIAP - Link Availability (Browser)', async ({ page }) => {
        const response = await page.goto('https://www.niap-ccevs.org/Product/', { waitUntil: 'domcontentloaded' });
        expect(response?.status()).toBe(200);
    });

    test('BSI Germany - Link Availability (Browser)', async ({ page }) => {
        const response = await page.goto('https://www.bsi.bund.de/EN/Topics/Certification_and_Validation/Certificates/CC_Products/cc_products_node.html', { waitUntil: 'domcontentloaded' });
        expect(response?.status()).toBe(200);
    });

    test('ANSSI France - Link Availability (Browser)', async ({ page }) => {
        const response = await page.goto('https://cyber.gouv.fr/en/certified-products', { waitUntil: 'domcontentloaded' });
        expect(response?.status()).toBe(200);
    });
