...
export const Dashboard: React.FC<DashboardProps> = ({ onSelectModule }) => {
  ...
  const ModuleCardContent = ({ module }: { module: ModuleItem }) => {
  ...
  const ModuleTable = ({ items, title, icon: Icon }: { items: ModuleItem[], title: string, icon: React.ElementType }) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 space-y-8">
        <ModuleTable items={activeModules} title="Learning Workshops" icon={BookOpen} />
        <ModuleTable items={upcomingModules} title="Upcoming Tracks" icon={Layers} />
      </div>
