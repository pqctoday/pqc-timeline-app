...
import { Trash2, Fingerprint, FlaskConical, HeartPulse, Activity, Building2 } from 'lucide-react'
import { HealthcarePQCIntroduction } from './components/HealthcarePQCIntroduction'
import { HealthcarePQCExercises, type WorkshopConfig } from './components/HealthcarePQCExercises'
...
export const HealthcarePQCModule: React.FC = () => {
  ...
  const handleReset = () => {
    if (confirm('Restart Healthcare PQC Module?')) {
      setCurrentPart(0)
      setConfigKey((prev) => prev + 1)
      startTimeRef.current = Date.now()
      updateModuleProgress(MODULE_ID, {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gradient">Healthcare PQC</h1>
          <p className="text-muted-foreground mt-2">
            Biometric data permanence, pharmaceutical IP protection, patient privacy lifecycles,
            medical device safety, and hospital network migration.
          </p>
        </div>
      </div>
        ...
        <TabsContent value="workshop">
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="glass-panel p-4 sm:p-6 md:p-8 min-h-[400px] md:min-h-[600px] animate-fade-in">
              ...
              />
              {currentPart === 0 && <BiometricVaultAssessor key={`biometric-${configKey}`} />}
              {currentPart === 1 && <PharmaIPCalculator key={`pharma-${configKey}`} />}
              {currentPart === 2 && <PatientPrivacyMapper key={`privacy-${configKey}`} />}
              {currentPart === 3 && <DeviceSafetySimulator key={`device-${configKey}`} />}
              {currentPart === 4 && <HospitalMigrationPlanner key={`migration-${configKey}`} />}
            </div>
