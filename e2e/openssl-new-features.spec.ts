...
test.describe('OpenSSL Studio - New Features (Enc, KEM, PKCS12)', () => {
    ...
    test('PKCS#12 Export and Import', async ({ page }) => {
        ...
        await page.waitForTimeout(500)
        const keyOption = page.locator('#csr-key-select option').filter({ hasText: 'rsa-2048-' }).first()
        ...
        // Select Cert
        await page.waitForTimeout(500)
        const certOption = page.locator('#p12-cert-select option').filter({ hasText: 'rsa-cert-' }).first()
        const certVal = await certOption.getAttribute('value')
        if (certVal) await page.selectOption('#p12-cert-select', certVal)

        // Select Key
        const p12KeyOption = page.locator('#p12-key-select option').filter({ hasText: 'rsa-2048-' }).first()
