...
const main = async () => {
    ...
    const runAll = args.includes('--all') || (!args.includes('--nist') && !args.includes('--acvp') && !args.includes('--cc') && !args.includes('--anssi'));
    ...
    if (currentData.length > 0 && !force) {
        ...
        if (age < STALE_THRESHOLD_MS) {
            console.log(`[Master Scraper] Data is fresh (${(age / 1000 / 60 / 60).toFixed(1)} hours old). Skipping scrape.`);
            console.log('Use --force to override.');
            return;
        } else {
            console.log(`[Master Scraper] Data is stale (${(age / 1000 / 60 / 60 / 24).toFixed(1)} days old). Refreshing.`);
        }
    }
    ...
    const normalizedData = [...keptData, ...newRecords].map(record => ({
        ...
        classicalAlgorithms: typeof record.classicalAlgorithms === 'string'
