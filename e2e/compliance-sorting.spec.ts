...
test.describe('Compliance Table Sorting', () => {
    test('should sort by Product Name', async ({ page }) => {
        ...
        const namesAsc = await page.locator('tbody tr td:nth-child(5) > div:first-child').allInnerTexts();
        // Verify sorted
        ...
        // Get all product names
        const namesDesc = await page.locator('tbody tr td:nth-child(5) > div:first-child').allInnerTexts();
        // Verify sorted desc
