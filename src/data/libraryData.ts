...
function parseLibraryCSV(csvContent: string): LibraryItem[] {
    ...
    const items: LibraryItem[] = lines.slice(1).map((line) => {
        ...
        } else if (type === 'protocol' || title.includes('tls') || title.includes('ssh') || title.includes('ikev2') || title.includes('cms')) {
            item.category = 'Protocols'
        } else if ((title.includes('key-encapsulation') || title.includes('kem')) && type === 'algorithm') {
            item.category = 'KEM'
        } else if ((title.includes('signature') || title.includes('dsa') || title.includes('sign')) && type === 'algorithm') {
