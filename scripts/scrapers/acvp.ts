    ...
    try {
        ...
        for (const group of algorithmGroups) {
            ...
            for (const row of candidateRows) {
                ...
                const moduleName = implLink?.textContent?.trim() || implCell?.textContent?.trim() || 'Unknown';
                ...
                const fullLink = relativeLink.startsWith('http') ? relativeLink : `https://csrc.nist.gov/projects/cryptographic-algorithm-validation-program/${relativeLink}`;
