        ...
        while (!stopScraping) {
            ...
            try {
                ...
                for (const item of items) {
                    ...
                    // Date
                    let dateStr = '';
                    const dateEl = Array.from(pageDoc.querySelectorAll('.field__label')).find(el => el.textContent?.includes('Date de fin de validité'))?.nextElementSibling
                        || Array.from(pageDoc.querySelectorAll('.field__label')).find(el => el.textContent?.includes('Publié le'))?.nextElementSibling; // Fallback

                    // Actually, "Publié le" is usually in metadata block
                    // Let's look for time tag or specific field
                    const dateNode = pageDoc.querySelector('time') || pageDoc.querySelector('.field--name-field-date-de-certification .field__item');
                    ...
                    const productName = urlSlug
                        ...
                        : (h1Title ? h1Title.replace(/\s+/g, ' ') : listTitle);
                    ...
                    if (certRef) {
                        ...
                        // Fallback ID
                        certId = `anssi-${productName.toLowerCase().replace(/[^a-z0-9]/g, '-').substring(0, 30)}-${Math.random().toString(36).substr(2, 4)}`;
                    }
                    ...
                    let pdfLink = allPdfLinks.find(a => {
                        ...
                        return href.includes('cible') || href.includes('security_target') || href.includes('st') ||
                            text.includes('cible') || text.includes('security target');
                    });

                    if (!pdfLink) {
                        pdfLink = allPdfLinks.find(a => a.textContent?.toLowerCase().includes('rapport') || a.getAttribute('href')?.includes('ANSSI'));
                    }
