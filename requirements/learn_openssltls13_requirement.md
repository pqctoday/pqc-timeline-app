...
- **FR-CRT-006:** The module MUST provide default pre-generated ML-DSA-44 certificates for both Client and Server identities to facilitate immediate PQC authentication testing without manual generation.

### 2.3.1 CA Trust Configuration

- **FR-TRUST-001:** The Client panel MUST display a "Verify Server Certificate" indicator (always-on) with a "Trusted Root CA" import section.
- **FR-TRUST-002:** The Server panel MUST display a "Root CA" import section when mTLS is enabled, to verify client certificates.
- **FR-TRUST-003:** Separate CA certificate paths MUST be maintained for client-to-server verification (`client-ca.crt`) and server-to-client verification (`server-ca.crt`).
- **FR-TRUST-004:** When switching between default identity types (RSA/ML-DSA), the corresponding CA certificates MUST be automatically set.

### 2.3.2 Identity Selection

- **FR-IDENT-001:** Both Client and Server panels MUST support identity selection via dropdown:
  - **Default (RSA 2048):** Pre-generated RSA certificates
  - **Default (ML-DSA-87):** Pre-generated ML-DSA post-quantum certificates
  - **Custom from OpenSSL Studio:** Import from application file store
- **FR-IDENT-002:** When "Custom from OpenSSL Studio" is selected, import buttons MUST be displayed for Certificate, Private Key, and Root CA.
- **FR-IDENT-003:** The identity selection MUST trigger automatic synchronization of the corresponding CA trust certificate.

### 2.4 Educational Output

- **FR-EDU-001:** The result output MUST highlight the negotiated Cipher Suite, Key Exchange Group, Signature Algorithm, and Protocol Version to provide clearly distinguishable visibility into the authentication method handling (e.g. RSA vs ML-DSA).
...
>
> - **EVP API** (key generation, signing): Uses hyphenated names like `ML-DSA-44`, `ML-KEM-768`
> - **TLS Config** (SignatureAlgorithms, Groups): Uses lowercase without hyphens like `mldsa44`, `mlkem768`
> - **Certificate Output** (OBJ_nid2sn): Returns OID-based names like `id-ml-dsa-44`
