...
test.describe('Library Feature', () => {
  ...
  test('should navigate to library page and show tabs', async ({ page }) => {
    // Check for main tabs
    await expect(page.getByRole('tab', { name: 'All' })).toBeVisible()
    await expect(page.getByRole('tab', { name: 'Digital Signature' })).toBeVisible()
    await expect(page.getByRole('tab', { name: 'KEM' })).toBeVisible()
  })
  ...
  test('should show details popup with correct info and layout', async ({ page }) => {
    ...
    // Verify 4-column grid layout for metadata
    await expect(popup.locator('.grid.grid-cols-1.md\\:grid-cols-4')).toBeVisible()
