...
export const content: ModuleContent = {
  ...
  narratives: {
    ...
    workshopSummary:
      'Eight interactive steps: (1) CPM Maturity Self-Assessment scoring the organization on a 5×4 grid (five pillars × four asset classes, rated 1–4 per CSWP.39 tier scale); (2) Inventory Lifecycle Simulator walking assets through the six-stage operational loop with canonical CLM scenarios (shadow-cert discovery, 47-day renewal, intermediate-CA rotation, OCSP drift); (3) Library & Hardware CBOM Builder combining SBOM→CBOM extraction, library version/EoL tracking, FIPS 140-3 Level 3 validation status, and SP 800-90B ESV status for libraries and HSMs — PQC migration is a trigger to verify entropy sources meet increased security-strength requirements; (4) No-Regret ROI Builder calculating ROI, NPV, and payback under quantum-happens and quantum-never-happens scenarios using outage avoidance, CLM automation, FIPS-drift remediation, library-CVE response, and CryptoCOE M&A readiness — every headline number expands into an editable sub-model with formula, parameters, source citation, and explanation; (5) Posture KPI Dashboard Designer for board-ready metrics across all five pillars; (6) Management Tools Coverage Audit rating tooling across the six CSWP.39 categories; (7) Risk Analysis Engine producing a prioritised asset list from the CBOM; (8) Mitigate vs Migrate Wizard mapping each asset to a CSWP.39 §4.6 response path.',
    entropyCompliance:
