...
describe('GSMA Audit: 5G SUCI Profiles via OpenSSL WASM', () => {
  ...
  beforeAll(async () => {
    ...
    openSSLService.execute.mockImplementation(
      async (cmd: string, files: { name: string; data: Uint8Array }[] = []) => {
  ...
  const runAuditTest = async (profile: 'A' | 'B' | 'C', pqcMode: 'hybrid' | 'pure' = 'hybrid') => {
    ...
    const ssRes = await fiveGService.computeSharedSecret(
      profile,
      ephRes.privKey,
      hnPubFile,
      pqcMode
    ...
    if (profile === 'A' || profile === 'B') {
      ...
      if (!zFileObj)
        throw new Error(
          `Audit Failed: No shared secret derived via OpenSSL for ${profile}\nUnknown error: ${verifyRes.stderr}`
        )
      ...
      if (pqcMode === 'pure') {
        ...
        if (!zEcdhObj)
          throw new Error(
            `Audit Failed: Hybrid ECDH failed. Stdout: ${resEcdh.stdout}\nStderr: ${resEcdh.stderr}`
          )
