...
export const scrapeNIST = async (): Promise<ComplianceRecord[]> => {
    try {
        const url = 'https://csrc.nist.gov/projects/cryptographic-module-validation-program/validated-modules/search/all?searchMode=Advanced&Standard=FIPS+140-3&ValidationStatus=Active&SecurityLevel=3';
        ...
        for (let i = 0; i < rows.length; i += BATCH_SIZE) {
            ...
            const batchPromises = batch.map(async (row) => {
                ...
                const certId = certLink?.textContent?.trim() || `nist-${Math.random().toString(36).substr(2, 5)}`;
                ...
                const fullLink = relativeLink.startsWith('http') ? relativeLink : `https://csrc.nist.gov${relativeLink}`;

                // detail fetch for PQC & Classical
                if (relativeLink) {
                    try {
                        const detailUrl = relativeLink.startsWith('http') ? relativeLink : `https://csrc.nist.gov${relativeLink}`;
                        ...
                        else if (moduleName.toLowerCase().includes('quantum') || moduleName.toLowerCase().includes('pqc')) {
