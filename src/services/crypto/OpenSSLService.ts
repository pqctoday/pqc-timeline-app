...
class OpenSSLService {
    ...
    public async init(): Promise<void> {
        ...
        this.readyPromise = new Promise((resolve, reject) => {
            try {
                ...
                this.worker = new Worker(new URL('../../components/OpenSSLStudio/worker/openssl.worker.ts', import.meta.url), {
                    type: 'classic',
                });
    ...
    public async execute(command: string, files: { name: string; data: Uint8Array }[] = []): Promise<OpenSSLCommandResult> {
        await this.init();
        ...
        const cmdStr = command.trim().startsWith('openssl ')
            ? command.trim().slice(8)
            : command.trim();
